...
radvs_request_queue_alloc_locked(RADVS_RequestQueue *queue, U64 message_size, U64 message_align)
{
  RADVS_Request *request = queue->free;
  if (request != 0) {
    SLLStackPop(queue->free);
    CondVar complete_cv = request->complete_cv;
    MemoryZero(request, message_size);
    request->complete_cv = complete_cv;
  } else {
    request = arena_push(queue->arena, message_size, Max(8, message_align), 1);
